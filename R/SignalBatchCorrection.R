...
SignalBatchCorrection <- function(QCreportObject)
{
  ...
  if (QCreportObject$Blank_label %in% class){
    blank_filtered <- 
      filter_peaks_by_blank(QCreportObject$peakMatrix, 20, class,
        blank_label=QCreportObject$Blank_label, qc_label = NULL, 
        remove_samples = FALSE, remove_peaks = TRUE)
    QCreportObject$filtering$table$`Number of features`[2:5] <- 
      nrow(blank_filtered)
  } else {
  ...
  PCAinF2 <- prepareData(Data=MV_filtered, classes=class,
    blank = QCreportObject$Blank_label, PQN=T, mv_impute = T, glogScaling = T,
  ...
  QCreportObject$plots$SBPCAbefore <- doPCA (Data=PCAinF2$Data, 
    classes=PCAinF2$classes, PQN=T, mv_impute = T, glogScaling = T, scale=F,
    labels=QCreportObject$pca_scores_labels, 
    qc_label = QCreportObject$QC_label, plotTitle = "PCA, filtered")


  QCreportObject$plots$SBPCAbeforeQC <- doPCA (Data=PCAinQC2$Data, 
    classes=PCAinQC2$classes, PQN=F, mv_impute = T, glogScaling = F, scale=T,
  ...
  if (length(nbatches) > 1){
    ...
    QCreportObject$plots$plots_per_batch_pca <- 
      plots_per_batch[seq(1, length(nbatches) * 2, 2)]
    QCreportObject$plots$plots_per_batch_rsd <- 
      plots_per_batch[seq(2, length(nbatches) * 2, 2)]
  ...
  SBcorrected <- pmp::QCRSC(df=MV_filtered, order=metaData$injection_order,
    batch=metaData$batch, classes=class, spar=0, minQC = 5)
  ...
  QCreportObject$plots$SBPCAfter <- 
    doPCA (Data=PCAinSB$Data, classes=PCAinSB$classes, PQN=T, mv_impute = T,
    glogScaling = T, scale=F, labels="none", 
  ...
  QCreportObject$plots$SBPCAfterQC <- doPCA (Data=PCAinSBQC$Data, 
    classes=PCAinSBQC$classes, PQN=F, mv_impute = T, glogScaling = F, scale= T,
    labels="QC", qc_label = QCreportObject$QC_label,
    plotTitle = "PCA, filtered and S/B corrected")
