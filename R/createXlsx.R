...
formatIonIdentifiers <- function(variableMetadata, numDigitsRT=0, 
  numDigitsMZ=0) {
    splitDeco <- strsplit(as.character(variableMetadata$name),"_")
    idsDeco <- sapply(splitDeco, function(x) { deco=unlist(x)[2]; 
    ...
    variableMetadata <- cbind(name=variableMetadata$name, namecustom=namecustom,
      variableMetadata[, !(colnames(variableMetadata) %in% c("name"))])
...
getPeaklistW4M <- function(xset, intval="into", convertRTMinute=F, 
  numDigitsMZ=4, numDigitsRT=0) {
...
createXlsx <- function (QCreportObject)
{
  ...
  } else
  {
    ...
    writeData (wb, "variableMetaData", data.frame(mz=names(variableMetaData), 
      intensity=as.vector(variableMetaData)), rowNames = F)
  }  
