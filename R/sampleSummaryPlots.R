...
sampleSummaryPlots <- function(QCreportObject)
{
  ...
  if (!all(is.na(A[ , 2])))
  {
    ...
    QCreportObject$tables$corrMatrix[1, ] <- c(round(corTICr$estimate, 2),
      corTICr$p.value)
    QCreportObject$tables$corrMatrix[2, ] <- c(round(corTICe$estimate, 2),
      corTICe$p.value)
    QCreportObject$tables$corrMatrix[3, ] <- c(round(corTICrTICe$estimate, 2),
      corTICrTICe$p.value)
