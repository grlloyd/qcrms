...
sampleSummary <- function (QCreportObject)
{
  ...
  if (!is.null(QCreportObject$metaData$file)){
    ...
    QCreportObject$metaData$table[,1] <- 
      as.vector(QCreportObject$metaData$table[ ,1])
    QCreportObject$metaData$table$Sample <- 
      rownames(QCreportObject$metaData$table)
    ...
    if (!is.null(QCreportObject$Blank_label)){
        bh <- grep(QCreportObject$Blank_label, 
          rownames(QCreportObject$metaData$table))
        if (length(bh)>0) QCreportObject$metaData$table[bh,1] <- 
          QCreportObject$Blank_label
      }
    if (!is.null(QCreportObject$QC_label)){
        qh <- grep(QCreportObject$QC_label, 
          rownames(QCreportObject$metaData$table))
        if (length(qh)>0) QCreportObject$metaData$table[qh,1] <- 
  ...
  # contain NA's.
  # checks if ClassQC column exists, to use that for Blank and QC sample indices.
    ...
    if (!all(file.exists(QCreportObject$raw_paths))){
      ...
      for (file in 1:length(files_to_locate)){
        ...
        QCreportObject$raw_paths[file] <- dir (QCreportObject$raw_path, 
          recursive = T, pattern = files_to_locate[file], full.names = T)
      }
    }
    ...
    for (i in 1:length(QCreportObject$raw_paths)){
      ...
      if(file.exists(QCreportObject$raw_paths[i])){
        ...
        QCreportObject$fileSize[i] <- 
          file.info(QCreportObject$raw_paths[i])$size / (1024 * 1024)
      }
    }
  }
  ...
  if (!is.null(QCreportObject$xset)){
    ...
    peaksDetected = as.vector(apply(QCreportObject$peakMatrix, 2, 
  ...
  QCreportObject$projectHeader <- rbind(QCreportObject$projectHeader,c("",""), 
    cbind(rNames, cValues))
  ...
  QCreportObject$Blank_hits <- which(QCreportObject$metaData$
      table[, QCreportObject$metaData$classColumn]==QCreportObject$Blank_label)
