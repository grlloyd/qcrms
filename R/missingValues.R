...
missingValues <- function (QCreportObject){
  ...
  for (slab in 1: length(cl)){
    ...
    out_across_features[[slab]] <- apply(cbind(QCreportObject$
      peakMatrix[ ,samp_lab==cl[slab]],NULL),1,
  ...
  QCreportObject$plots$MVplot3 <- ggplot (data=across_samples, 
    aes_(x=~x, y=~class, color=~x)) + geom_boxplot(show.legend = F) +
    ylab("missing values, %") +
    ylim (0,100)+ xlab("") +
    scale_colour_manual(values=QCreportObject$plots$MVplotClassS$manual_colors) +
  ...
  QCreportObject$plots$MVplot4 <- ggplot (data=across_features, 
    ...
    ylab("missing values, %") +
    xlab("") + 
    scale_colour_manual(values=QCreportObject$plots$MVplotClassF$manual_colors) +
    theme_Publication(base_size = 12)+
    ylim (0,100)+
    coord_flip()
